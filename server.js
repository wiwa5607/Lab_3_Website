...
app.get('/player_info/post', function (req, res) {
	...
	var selected = "select * from football_players where name = '" + select + "';";
	// console.log(selected);
	var games = "SELECT COUNT(football_games.players) FROM football_games INNER JOIN football_players ON (SELECT id FROM football_players WHERE name = '" + select + "') = ANY (football_games.players) WHERE football_players.name = '" + select + "' ;";
	// console.log(games);
	db.task('get-everything', task => {
		return task.batch([
			task.any(players),
		...
		.then(data => {
			// console.log("The Player's name is");
			// console.log(data[1]);
			// console.log(data);
			res.render('pages/player_info', {
				my_title: 'Player Info',
...
app.post('/player_info/post', function (req, res) {
	...
	var select = req.query.player_choice;
	var selected = "select * from football_players where name = '" + select + "';";
	var games = "SELECT COUNT(football_games.players) FROM football_games INNER JOIN football_players ON (select id from football_players where name = '" + select + "') = ANY (football_games.players) WHERE football_players.name = '" + selected + "' ;";
	db.task('get-everything', task => {
		return task.batch([
			task.any(players),
			task.any(selected),
